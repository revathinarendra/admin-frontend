...
// import { useEffect, useState } from 'react';
// import {
//   Box, Typography, Paper, TextField, Button, Grid, Snackbar, Alert, Stack
// } from '@mui/material';
...
// const ItemsPage = () => {
//   const [items, setItems] = useState([]);
//   const [cartItems, setCartItems] = useState<number[]>([]); // Store item IDs already in cart
//   const [title, setTitle] = useState('');
//   const [description, setDescription] = useState('');
...
//       const cartItemIds = res.data.map((item: any) => item.item); // get item IDs from cart
...
//     } catch (err: any) {
//       setSnackbar({ open: true, message: err.response?.data?.error || 'Failed to add to cart', severity: 'error' });
//     }
//   };
...
//             <TextField fullWidth label="Title" value={title} onChange={e => setTitle(e.target.value)} margin="normal" />
//             <TextField fullWidth label="Description" value={description} onChange={e => setDescription(e.target.value)} margin="normal" multiline />
//             <Button variant="contained" onClick={handleAddItem} sx={{ mt: 2 }}>Add Item</Button>
//           </Paper>
//         </Grid>
...
//               <Typography variant="h6">Your Items</Typography>
//               <Button variant="outlined" onClick={handleViewCart}>View Cart</Button>
//             </Stack>
//             {items.map((item: any) => {
//               const alreadyInCart = cartItems.includes(item.id);
//               return (
//                 <Box key={item.id} mb={3} mt={2} p={2} border="1px solid #ccc" borderRadius={2}>
//                   <Typography fontWeight="bold">{item.title}</Typography>
